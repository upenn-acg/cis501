...
# create a zip archive of source code and a bitstream. We filter out warnings because for the ALU-only version of the processor labs we pull in a bitstream, even though the bitstream is only for the full version of the lab
zip: $(ZIP_SOURCES)
	zip $(ZIP_FILE) $(ZIP_SOURCES) | grep -v warning

# create a zip archive of power/performance/area reports generated by `make impl`
ppa: output/post_route_power_report.txt output/post_route_timing_summary_report.txt output/post_route_utilization_report.txt
	zip $@.zip $^
